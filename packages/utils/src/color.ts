...
const hexToRgb = (color: string): [number, number, number] => {
  ...
  if (!values) {
    throw new Error(`KontUI: Unsupported ${color} color.`)
  }
  return [
...
const colorToRgbValues = (color: string) => {
  if (color.charAt(0) === "#") return hexToRgb(color)

  const safeColor = color.replace(/ /g, "")
  const colorType = color.slice(0, 4)

  const regArray = safeColor.match(/\((.+)\)/)
  if (!colorType.startsWith("rgb") || !regArray) {
    console.log(color)
    throw new Error(`KontUI: Only support ["RGB", "RGBA", "HEX"] color.`)
  }

  return regArray[1].split(",").map((str) => Number.parseFloat(str))
}

const addColorAlpha = (color: string, alpha: number) => {
