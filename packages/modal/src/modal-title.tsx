import * as React from "react"
import { useTheme, useScale, withScale } from "@kontui/theme"
import { __DEV__ } from "@kontui/utils"

import type { DefaultProps } from "@kontui/theme"

interface ModalTitleProps extends DefaultProps {}

const ModalTitleBase: React.FunctionComponent<ModalTitleProps> = (props) => {
  const { className, children, ...rest } = props

  const theme = useTheme()
  const { SCALES } = useScale()

  return (
    <>
      <h2 className={className} {...rest}>
        {children}
      </h2>
      <style jsx>{`
        h2 {
          line-height: 1.6;
          font-weight: normal;
          text-align: center;
          display: inline-flex;
          flex-shrink: 0;
          justify-content: center;
          align-items: center;
          word-break: break-word;
          text-transform: capitalize;
          font-size: ${SCALES.font(1.5)};
          color: ${theme.palette.foreground};
          width: ${SCALES.width(1, "auto")};
          height: ${SCALES.height(1, "auto")};
          padding: ${SCALES.pt(0)} ${SCALES.pr(0)} ${SCALES.pb(0)}
            ${SCALES.pl(0)};
          margin: ${SCALES.mt(0)} ${SCALES.mr(0)} ${SCALES.mb(0)}
            ${SCALES.ml(0)};
        }
      `}</style>
    </>
  )
}

const ModalTitle = withScale(ModalTitleBase)

if (__DEV__) {
  ModalTitle.displayName = "ModalTitle"
}

export type { ModalTitleProps }
export { ModalTitle }
